<script lang="ts">
	...
	import SettingsModal from '../Views/Settings/SettingsModal.svelte';
	...
	import KnightPortrait from "$lib/assets/portraits/Knight.png"
</script>
...
<div class="grid h-full w-full grid-cols-3 grid-rows-6 p-8">
	...
	<div class="col-start-3 flex items-start justify-end space-x-6 [&>*]:scale-150">
		...
		<Button onclick={() => openLayout(AvailableLayouts.Character)} size="icon" variant="ghost" class="rounded-full border overflow-clip border-muted-foreground">
			<img src={KnightPortrait} alt="Portrait" />
		</Button>
	</div>
	<div class="col-start-2 row-start-6 flex flex-col items-center justify-center">
		<div class="mx-auto w-full max-w-[300px]">
			<div class="flex w-full items-center space-x-2">
				<div class="block">Lv. {gameState.character!.level}</div>
				<div
					class="h-2 flex-grow rounded border"
					style="background: linear-gradient(0.25turn, green 100%, black 100%);"
				></div>
			</div>
		</div>
	</div>
